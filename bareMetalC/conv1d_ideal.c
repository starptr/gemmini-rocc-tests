...
#define KER_LEN 2 // this is codesynthed from runner, not synthesized
void runner(elem_t In[KER_LEN][LEN], elem_t Out[1][SLIDES]) {
  //static elem_t In[KER_LEN][LEN];
  //static elem_t Out[1][SLIDES];

  static elem_t weights[KER_LEN][KER_LEN];
  //weights[0][j] = 1; // kernel values here
		       // ...
  // placeholder:
  weights[0][0] = 1;
  weights[0][1] = 1;
  // end placeholder
  for (int i = 1; i < KER_LEN; i++) {
